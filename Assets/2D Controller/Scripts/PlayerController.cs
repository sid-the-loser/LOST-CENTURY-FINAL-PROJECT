...
namespace TarodevController
{
    ...
    public class PlayerController : MonoBehaviour, IPlayerController, IPhysicsObject
    {
        ...
        public AudioSource _musicPresent;
        public AudioSource _musicPast;
        public AudioSource _timeJumpSound;
        public AudioSource _swordSwing;
        public AudioSource _swordHit;
        public AudioSource _levelEnd;
        public AudioSource _levelStart;


        #endregion

        #region PlayerAttacksMech

        [SerializeField] private bool swordInhand = true;
        ...
        [SerializeField] private GameObject bottomAttackCollider;
        
        [SerializeField] private HealthBar healthBar;
        
        private float swordSpawnOffset = 1f;
        //public float randNumForPitchSound = System.Random.Range(0.5f, 2.5f);

        private GameObject _tempSwordProjectile;
        private SwordProjectile _tempSwordProjectileLogic;
        ...
        private float stompForce = 50;

        private float health = 100f;
        private float monsterDamage = 2.5f;

        public void SetSwordInhand(bool value)
        {
            swordInhand = value;
        }

        public bool GetSwordInhand()
        {
            return swordInhand;
        }
        ...
        private void Start()
        {
            ...
            //Animation
            FindObjectOfType<PlayerAnimator>()._anim.SetBool("_ending", false);
        ...
        private void Update()
        {
            ...
            {
                print("Ded");
                FindObjectOfType<GameManager>().ReloadCurrentScene();
            }
            
            if (_frameDirection.x != 0) _attackDirection = _frameDirection;
            
            if (!swordInhand)
            {
                //Animation
                FindObjectOfType<PlayerAnimator>()._anim.SetBool("_swordin", false);
                if (_tempSwordProjectileLogic is not null)
                {
                    if (_tempSwordProjectileLogic.collided)
                    {
                        //Audio
                        // _swordHit.Play();  not sure whether to add this here or on the enemy script (when collides with enemy, makes hit sound)
                        _swordSwing.Play();

                        Destroy(_tempSwordProjectile);

                        //Animation
                        FindObjectOfType<PlayerAnimator>()._anim.SetBool("_swordin", true);
                        swordInhand = true;
                    }
                }
            }
            ...
            if (swordInhand)
            {
                // ANI: Player holding sword; idle animation
                FindObjectOfType<PlayerAnimator>()._anim.SetBool("_swordin", true);
            }
            else
            {
                //Animations
                FindObjectOfType<PlayerAnimator>()._anim.SetBool("_swordin", false);
                FindObjectOfType<PlayerAnimator>()._anim.SetBool("_isAttacking", false);
                FindObjectOfType<PlayerAnimator>()._anim.SetBool("_stompin", false);
                FindObjectOfType<PlayerAnimator>()._anim.SetBool("_timeTravelin", false);
            }
        }
        /*
        ...
        */
        ...
        private void Attack()
        {
            if (swordInhand)
            {
                ...
                {
                    swordInhand = false;
                    _tempSwordProjectile = Instantiate(swordProjectile, new Vector3(transform.position.x + swordSpawnOffset, transform.position.y + swordSpawnOffset, transform.position.z), Quaternion.Euler(0, 0, _lookAtAngle+270));
                    _tempSwordProjectileLogic = _tempSwordProjectile.GetComponent<SwordProjectile>();
                    _tempSwordProjectileLogic.projAngle = _lookAtAngle;
                    _tempSwordProjectileLogic.LaunchSword();

                        // SFX: Sword throw sfx
                }
                else
                {
                    StopCoroutine(TurnOffAttackColliders());
                    ...
                    StartCoroutine(TurnOffAttackColliders());
                    // ANI: Player holding sword; melee attack animation
                    // SFX: Sword strike sfx
                }*/
                StopCoroutine(TimerTurnOffAttackColliders());

                if (_attackDirection.x > 0)
                {
                    rightAttackCollider.SetActive(true);
                    leftAttackCollider.SetActive(false);
                    bottomAttackCollider.SetActive(false);
                }
                else
                {
                    rightAttackCollider.SetActive(false);
                ...
                StartCoroutine(TimerTurnOffAttackColliders());

                //Animation
                FindObjectOfType<PlayerAnimator>()._anim.SetBool("_isAttacking", true);

                //_swordSwing.pitch = randNumForPitchSound;

                //Audio
                _swordSwing.pitch = 1.2f;
                _swordSwing.Play();
            }
        }
        ...
        private void Stomp()
        {
            if (!_grounded && swordInhand)
            {
                StopCoroutine(TimerTurnOffAttackColliders());
                
                rightAttackCollider.SetActive(false);
                leftAttackCollider.SetActive(false);
                bottomAttackCollider.SetActive(true);

                //Animation
                FindObjectOfType<PlayerAnimator>()._anim.SetBool("_stompin", true);


                _rb.AddForce(Vector2.up * 10, ForceMode2D.Impulse); //I wanted to add a small vertical boost before the stomp (optional..just for visuals) - Harrish. 

                _rb.AddForce(Vector2.down * stompForce, ForceMode2D.Impulse);

                //Audio
                _swordSwing.pitch = 0.8f;
                _swordSwing.Play();
            }
        ...
        IEnumerator TimeJump()
        {
            ...
            {
                //Animation
                FindObjectOfType<PlayerAnimator>()._anim.SetBool("_timeTravelin", true);
                ...
                if (_istutorial)
                {
                    _musicPresent.volume = 0.65f;
                }
                else
                {
                    if (_musicPresent.volume == 0.65f)
                    {
                        _musicPresent.volume = 0f;
                        _musicPast.volume = 0.65f;
                    }
                    else
                    {
                        _musicPresent.volume = 0.65f;
                        _musicPast.volume = 0f;
                    }
                }
                ...
                //InsertTimeJumpCodeHere
            }

        }
        
        private IEnumerator TimerTurnOffAttackColliders()
        {
            yield return new WaitForSeconds(_attackDuration);
            TurnOffAttackColliders();

            //Animation
            FindObjectOfType<PlayerAnimator>()._anim.SetBool("_isAttacking", false);
        }

        private void TurnOffAttackColliders()
        {
            rightAttackCollider.SetActive(false);
            leftAttackCollider.SetActive(false);
            bottomAttackCollider.SetActive(false);
        ...
        private void Throw()
        {
            ...
            {
                swordInhand = false;
                _tempSwordProjectile = Instantiate(swordProjectile,
                    new Vector3(transform.position.x, transform.position.y + swordSpawnOffset,
                        transform.position.z), Quaternion.Euler(0, 0, _lookAtAngle + 270));
                _tempSwordProjectileLogic = _tempSwordProjectile.GetComponent<SwordProjectile>();
                _tempSwordProjectileLogic.projAngle = _lookAtAngle;
                _tempSwordProjectileLogic.LaunchSword();
        ...
        #region TimeTravelMech

        [SerializeField] private float pastLevelOffset;
        
        private bool currentlyInPresent = true;

        public void TimeTravel()
        {
            if (swordInhand)
            {
                if (currentlyInPresent)
                {
                    _musicPast.volume = 0.65f;
                    _musicPresent.volume = 0f;
                    transform.position += new Vector3(0, pastLevelOffset, 0);
                    if (_istutorial)
                    {
                        _musicPast.Play();
                        FindObjectOfType<GameManager>().sealsBroken = 3;
                    }
                }
                else
                {
                ...
                currentlyInPresent = !currentlyInPresent;
            }
        }
        ...
        private void GatherInput()
        {
            ...
            if (_frameInput.Throw)
            {
                Throw();
            }

            if (_frameInput.Stomp)
            {
                Stomp();
            }
            ...
            var tempLookAtAxis = _frameInput.LookAxis;
            if (tempLookAtAxis.y != 0 || tempLookAtAxis.x != 0)
            {
                _lookAtAngle = (float)Math.Atan2(tempLookAtAxis.y, tempLookAtAxis.x) * Mathf.Rad2Deg;
            }
            else
            {
                if (_attackDirection.x > 0)
        ...
        private void CalculateCollisions()
        {
            ...
            if (isGroundedThisFrame && !_grounded)
            {
                ToggleGrounded(true);
                TurnOffAttackColliders();
        ...
        private void CalculateDirection()
        {
            ...
            if (_frameDirection != Vector2.zero && _grounded)
            {
                _movin = true;
            }
            else
