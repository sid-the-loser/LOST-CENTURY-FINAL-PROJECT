...
namespace TarodevController
{
    public class PlayerAnimator : MonoBehaviour
    {
        ...
        private void OnWallGrabChanged(bool onWall)
        {
            ...
            if (_isOnWall)
            {
                _anim.SetBool("_wallin", true);
                PlaySound(_wallGrabClip, 0.5f);
            }
            else
            {
                _anim.SetBool("_wallin", false);
        ...
        private void HandleWallSlideEffects()
        {
            ...
            if ((_player.ClimbingLadder || _isOnWall) && _player.Velocity.y > 0) 
                //I currently do not have a ladder climbing animation - Harrish
            {
        ...
        private void HandleCharacterTilt(float xInput)
        {
            ...
            if (xInput != 0)
            {
                _anim.SetBool("_movin", true);
            }
            else
            {
                _anim.SetBool("_movin", false);
            }
